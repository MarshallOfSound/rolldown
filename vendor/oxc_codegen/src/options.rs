...
pub struct CodegenOptions {
    ...
    /// identifiers are escaped (`\uXXXX`, surrogate pairs above the BMP). Tagged template
    /// quasis are left as written (their raw value is observable) and so are comments.
    ///
    /// Default is `false`.
    pub ascii_only: bool,
