...
pub(crate) enum NonAsciiEscape {
    /// An IdentifierName: `\uXXXX`, or `\u{X…}` above the BMP.
    Identifier,
    ...
    /// escaped LS/PS line continuation is dropped, `</script` is escaped).
    TemplateRaw,
}
...
impl<'a> Codegen<'a> {
    ...
    ///
    /// Each escape in an IdentifierName must denote one whole code point, so characters
    /// above the BMP are written as `\u{XXXXX}` here (a surrogate pair is only joined back
    /// into one code point inside string/template literals and `u`-mode regular expressions).
    #[inline]
    ...
    pub(crate) fn print_non_ascii_escaped(&mut self, s: &str, mode: NonAsciiEscape) {
        let raw = matches!(mode, NonAsciiEscape::RegExp | NonAsciiEscape::TemplateRaw);
        let mut start = 0;
        for (i, ch) in s.char_indices() {
            ...
            // `\u00E9` mean the same in both grammars, and a template LineContinuation
            // contributes nothing to the cooked value, so it is dropped entirely.
            let mut end = i;
            ...
            let line_continuation = escaped
                && mode == NonAsciiEscape::TemplateRaw
                && matches!(ch, '\u{2028}' | '\u{2029}');
            if line_continuation {
                continue;
            }
            match mode {
                NonAsciiEscape::Identifier if (ch as u32) > 0xFFFF => {
                    self.print_code_point_escape(ch);
                }
                _ => self.print_unicode_escape(ch),
            }
        }
        if start < s.len() {
            if mode == NonAsciiEscape::TemplateRaw {
    ...
    /// Print `ch` as `\uXXXX`, or as an escaped surrogate pair when above the BMP (valid and
    /// equivalent in string literals, template literals and regular expressions with or
    /// without the `u`/`v` flag).
    #[inline]
    pub(crate) fn print_unicode_escape(&mut self, ch: char) {
        let cp = ch as u32;
        if cp <= 0xFFFF {
            self.print_u16_escape(cp as u16);
        } else {
            let v = cp - 0x1_0000;
            self.print_u16_escape(0xD800 + (v >> 10) as u16);
            self.print_u16_escape(0xDC00 + (v & 0x3FF) as u16);
        }
    }

    /// Print `ch` as `\u{X…}` (ES2015 code point escape) — the only escape form that can
    /// denote a supplementary-plane character in an IdentifierName.
    #[inline]
    fn print_code_point_escape(&mut self, ch: char) {
