...
impl PrintStringState<'_> {
    ...
    fn calculate_quote_maybe_backtick(&self) -> Quote {
        ...
        // 2. Single quote
        // 3. Backtick
        // A template literal is only chosen when it is strictly shorter (it saves escaping a
        // newline or both quote kinds): on a tie the plain string literal is what every other
        // minifier emits, what `import("./x.js")`-scanning tooling expects, and what keeps a
        // string in directive/property-key-compatible form.
        #[rustfmt::skip]
