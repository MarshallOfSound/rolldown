...
impl GenerateStage<'_> {
  pub(super) fn optimize_dynamic_entry_bits(
    ...
    // The static-cycle veto below asks, for every candidate reduction, whether the atom→chunk
    // graph (atoms grouped by identical dependent-entry bits, edges from `cycle`) would contain
    // a cycle. Rebuilding that whole graph per candidate is O(atoms × edges) per atom — seconds
    // on a large app. `IncrementalGroupGraph` keeps the grouping and the group-level edge
    // multiset up to date as atoms move between groups, so each candidate costs only the moved
    // atom's incident edges plus one reachability probe from its new group (a move only ADDS
    // edges incident to the destination group, so any new cycle passes through it). That probe
    // is exact only while the graph is known acyclic, so if the initial graph already has a
    // cycle we keep the original whole-graph check.
    ...
    for atom_idx in 0..atoms.len() {
      ...
      let creates_static_cycle = |atoms: &[ChunkAtom], incremental: &mut IncrementalGroupGraph| {
        if use_incremental {
          // Tentatively move the atom to the group of its reduced bits; keep the move only if
          // it stays acyclic (the caller restores the bits otherwise, matching the graph).
          incremental.move_atom(atom_idx, &atoms[atom_idx].dependent_entries);
          if incremental.group_reaches_itself(incremental.group_of(atom_idx)) {
...
/// Atoms grouped by identical `dependent_entries` (each group is a chunk-to-be) with the
/// group-level dependency multigraph maintained incrementally, for the static-cycle veto in
/// `optimize_dynamic_entry_bits`. Group ids are never reused; a group left empty simply has no
/// edges.
struct IncrementalGroupGraph {
  group_by_bits: FxHashMap<BitSet, usize>,
  atom_group: Vec<usize>,
  /// atom → atoms it depends on / atoms depending on it (the `cycle` graph and its reverse).
  deps: Vec<Vec<usize>>,
  rdeps: Vec<Vec<usize>>,
...
impl IncrementalGroupGraph {
  ...
  /// Re-home `atom_idx` in the group for `bits` (creating it if new), moving its incident
  /// edges with it.
  ...
  /// Whether `group` lies on a cycle (DFS over its successors looking for itself).
  ...
  /// Full check, used once up front (same DFS as `chunk_dependency_graph_has_cycle`).
