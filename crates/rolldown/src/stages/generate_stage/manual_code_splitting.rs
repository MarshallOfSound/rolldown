...
impl ManualSplitter<'_> {
  ...
  async fn build_module_groups(
    ...
    /// How many callback invocations are in flight at once (bounded so the JS side's
    /// queues stay small).
    ...
    // User-supplied `test` / `name` functions live on the JavaScript side: awaiting them one
    // (module, group) pair at a time costs a full main-thread round trip each — tens of
    // thousands of sequential hops on a large app. Evaluate them up front instead and let the
    // loop below read the cached answers. Calls are dispatched one callback function at a
    // time (every module for group 0's `test`, then group 1's, …, then the `name` callbacks
    // the same way), each function's calls in the deterministic module order above: a
    // JavaScript function's queued calls run in dispatch order, so a stateful callback still
    // observes the same sequence on every run. (Different functions are never in flight at
    // the same time, since the host may drain their queues independently.)
    ...
    let passes_static_filters =
      |match_group_index: usize, match_group: &MatchGroup, normal_module: &rolldown_common::NormalModule| {
    ...
    for (match_group_index, match_group) in self.match_groups.iter().copied().enumerate() {
      ...
      for batch in candidates.chunks(CALLBACK_BATCH) {
        let results = futures::future::try_join_all(
          batch.iter().map(|m| match_group.name.value(&ctx, &m.id)),
        )
        .await?;
