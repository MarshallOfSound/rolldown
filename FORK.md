...
| `fix: derive reference ids from stable seeds`                              | file emitter   | reference ids were hashes of a thread-order-dependent emission counter whenever no name was given (assets without an explicit `fileName`, unnamed emitted chunks), making Vite's `__VITE_ASSET__<id>__` placeholders (and anything hashed over pre-render chunk code, e.g. Sentry debug ids) nondeterministic across identical builds; the counter is gone and every id is a pure function of a stable seed (content hash for assets, module id for unnamed chunks, file name otherwise). Ids are internal/pre-render, so final bundle output is unchanged — just reproducible. Same change as [rolldown#10797](https://github.com/rolldown/rolldown/pull/10797).                                                                  |
